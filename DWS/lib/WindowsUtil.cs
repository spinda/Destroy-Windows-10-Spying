...
namespace DWS_Lite.lib
{
    internal class WindowsUtil
    {
        public static object ReadSubKeyValue(string subKey, string keyName)
        {
            ...
            if (rkSubKey == null)
            {
                MessageBox.Show($@"Error while reading registry key: {subKey}\{keyName} does not exist!", @"Error",
            ...
            {
                MessageBox.Show(
                    $"Error while reading registry key: {subKey} param: {keyName}. ErrorMessage: {ex.Message}", @"Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                rkSubKey.Close();
                return null;
            }
        ...
        {
            return
                Convert.ToInt32(ReadSubKeyValue(@"SOFTWARE\Microsoft\Windows NT\CurrentVersion\", "CurrentBuildNumber"));
        }

        public static bool UAC_Status()
        {
            return
                !Convert.ToBoolean(ReadSubKeyValue(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\",
                    "EnableLUA"));
        }
        ...
        {
            return
                Convert.ToInt32(ReadSubKeyValue(@"SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore\",
                    "RPSessionInterval"));
        }
